...
> wired to a `_shared/versions.tf`; only `templates/tofu-stack/` uses the symlinked-SSOT
...
> root `.opentofu-version` is the one true live SSOT.
...
| A3 | Shared partial-backend config | `infra/state-foundation/backend.hcl` → `bucket`/`region`/`dynamodb_table`/`kms_key_id`/`encrypt` (lines 10–14) | Hardcoded AWS identifiers populated from `bootstrap` outputs; Renovate doesn't touch `.hcl` backend config | After `bootstrap`, paste real values from `tofu output`; update on account/region/KMS change |
...
| A12 | `required_version` floor | `bootstrap/main.tf` (line 16), `foundation/backend.tf` (line 9), each module `main.tf`, `consumers/seqtoid-web/backend.tf` (line 10) — all `>= 1.6` | Hand-set OpenTofu floor, duplicated across stacks; distinct from `.opentofu-version` (B1) | Bump by hand if a stack needs a higher floor; keep consistent |
| A13 | CI `validate` stack matrix | `.github/workflows/tofu-ci.yml` → `jobs.validate.strategy.matrix.stack` (lines 42–45) | Bespoke list of stack dirs to validate | Add the new stack path by hand when stacks are added |
...
|---|------|--------------------------------|---------------|
| B1 | OpenTofu version (root SSOT) | `.opentofu-version` → `1.12.1` | Renovate custom regex manager (`renovate.json`, `depName opentofu/opentofu`, github-releases); also read by CI via `tofu_version_file` in `tofu-ci.yml` |
| B2 | OpenTofu version (template SSOT) | `templates/tofu-stack/.opentofu-version` | `templates/tofu-stack/renovate.json` custom regex manager (when the template ships as its own Renovate-enabled repo) |
| B3 | Provider version **constraints** | `foundation/versions.tf` `aws`/`tls` `version`; each module `main.tf` `required_providers` `version`; `templates/tofu-stack/_shared/versions.tf` | Renovate `terraform` manager, grouped "terraform providers" (`renovate.json` packageRules) |
| B4 | Committed provider lockfiles | `infra/state-foundation/foundation/.terraform.lock.hcl`; `…/bootstrap/.terraform.lock.hcl`; `templates/tofu-stack/envs/dev/example/.terraform.lock.hcl` | Renovate terraform manager updates lock hashes alongside constraint bumps. NOTE: `:maintainLockFilesDisabled` is set — locks move only *with* a constraint bump, not on their own |
| B5 | GitHub Actions `uses:` pins | `.github/workflows/tofu-ci.yml` (`actions/checkout@v6`, `opentofu/setup-opentofu@v2`); `.github/workflows/security.yml` (`actions/checkout@v6`, `aquasecurity/trivy-action`, `terraform-linters/setup-tflint`, `bridgecrewio/checkov-action`) | Renovate `github-actions` manager, grouped "github actions" |
...
- New stack? Add its backend `key` (A2) + `terraform_remote_state` (A4), and add the dir to the `tofu-ci.yml` validate matrix (A13).
